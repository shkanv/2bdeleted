...
[theta, cost] = fminunc(@(t)(costFunction(t, X_, y)), initial_theta, options);
...
[x_topadmm, history_topadmm] = l1_logreg_top_admm(A, b, mu/15, 1, 1, step_size__x, step_size__z);
[x_topadmm_, history_topadmm_] = l1_logreg_top_admm__neg_dual_var(A, b, mu/50, 1.25, 1.25, step_size__x, step_size__z);
...
plot(cumsum(history_admm.total_iter), history_admm.objval, 'r--', 'MarkerSize', 10, 'LineWidth', 2);
...
semilogy(cumsum(history_admm.total_iter), max(1e-8, history_admm.r_norm), 'r', ...
    cumsum(history_admm.total_iter), history_admm.eps_pri, 'r--',  'LineWidth', 2);
...
semilogy(cumsum(history_admm.total_iter), max(1e-8, history_admm.s_norm), 'r', ...
    cumsum(history_admm.total_iter), history_admm.eps_dual, 'r--', 'LineWidth', 2);
...
y_true_0_ind = find(y_true==0);
y_true_1_ind = find(y_true==1);


%subplot(3,1,1)
hold all;
scatter3(X_full(:,1), X_full(:,2), 2*y_true-1, 'd'); 
%subplot(3,1,2)
