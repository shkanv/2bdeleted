function [z, history] = l1_logreg_top_admm__neg_dual_var(A, b, mu, rho, alpha, step_size__x_m, step_size__z)
...
for k = 1:MAX_ITER
    ...
    % x-update
    if 0
        x = update_x__quasi_newton(A, b, u, z, rho);
    else
        %x              = x_update_top_admm(A, b, u, z, rho, x_prev, step_size__x_m);
        grad_at_xold   = grad_log_reg(A, b, xold); % == g = C'*(exp(C*x)./(1 + exp(C*x)));
               
        x              = z - step_size__x_m.*grad_at_xold + u;
    end

    % z-update with relaxation
    zold     = z;
    x_hat    = alpha*x + (1-alpha)*zold;
    z        = x_hat - u - step_size__z.*grad_log_reg(A, b, zold);
...
function x = x_update_top_admm(A, b, u, z, rho, x_prev, step_size)

    grad_at_x_prev = grad_log_reg(A, b, x_prev);
    x              = z - step_size.*grad_at_x_prev - u;

end



function x = update_x__quasi_newton(A, b, u, z, rho, x0)
    % solve the x update
